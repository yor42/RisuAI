...
**Status 2026-09-21:** stage 7a (stop deleting: startup asset sweep and manual cleanup) is **committed `c66c9f4b`** (Report 13 §2; gates ledger 29/33/34). Stage 7b's minimal core (a failed load changes nothing, send guards, a failure notice) is **committed `3e17c8a3`** (gates ledger 39/40). Stage 7c (recovery, telling missing data from read errors, plugin storage, plus the items moved from 7b) is designed and still to do.
