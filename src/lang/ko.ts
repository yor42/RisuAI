export const languageKorean = {
    ...
    "errors": {
        ...
        "coldStorageChatLoadFailed": (coldDataKey: string) =>
            `이 채팅의 저장된 데이터를 불러올 수 없습니다 (키: ${coldDataKey}). 일시적인 문제일 수 있습니다 — 채팅을 전환하거나 앱을 재시작해보세요. 계속 실패한다면 데이터가 손실되었을 수 있으며, 이 채팅을 삭제할 수 있습니다. 그 전까지는 "사용하지 않는 콜드 스토리지 정리"를 실행하지 마세요.`,
        "coldStorageChatDataMissing": (coldDataKey: string) =>
            `이 채팅의 저장된 데이터를 찾을 수 없습니다 (키: ${coldDataKey}). 다른 기기에서 백업이 있다면 먼저 복원하세요. 그렇지 않다면 데이터가 손실된 것이며, 이 채팅을 삭제할 수 있습니다.`,
    ...
    "help": {
        ...
        "localNetworkModeDesc": "브라우저에서 직접 요청하는 대신, 로컬/사설망 모델 URL을 로컬 런타임 경로로 우회시킵니다.\n\n**목적**\n- `192.168.x.x`, `10.x.x.x`, `localhost`, `.local` 등 로컬 호스트에 대한 브라우저의 사설망/CORS 제한을 회피합니다.\n- Node 셀프 호스트 모드에서 로컬 추론의 첫 토큰이 느릴 때 발생하는 타임아웃 위험을 줄입니다.\n\n**작동 방식**\n- 로컬 네트워크 모드가 활성화되어 있고 대상 URL이 로컬/사설망으로 감지된 경우에만 적용됩니다.\n- Node 셀프 호스트: 스트리밍은 실험적인 Job+WebSocket 릴레이를 먼저 사용하고 (실패 시 `/proxy2`로 대체), 비스트리밍은 `/proxy2`를 사용합니다.\n- Tauri: 네이티브/직접 경로를 사용합니다.\n- 공개 웹 모드: 설계상 로컬/사설망 직접 호출이 차단됩니다.\n\n**제약**\n- OpenAI 호환 요청 경로에만 적용됩니다.\n- 두 공개 도메인 간의 Cloudflare 오리진 제한을 우회하지는 않습니다.\n- 이 기능이 작동하려면 (`globalThis.__NODE__ === true`인) 셀프 호스트 URL을 사용해야 합니다.",
        ...
        "keepSessionAlive": "탭을 활성 상태로 유지하여 브라우저에서 비활성으로 인해 세션이 만료되는 것을 방지합니다. 적용하려면 새로고침이 필요할 수 있습니다.\n\n- **소리로 유지**: 일정한 간격으로 무음 오디오를 재생하여 세션을 유지합니다. 대부분의 브라우저에서 가장 호환성이 좋고 효과적인 방법으로 알려져 있습니다.\n",
    ...
    "setup": {
        ...
        "claudeDesc": "Claude는 Anthropic에서 만든 AI 모델로, 영어 이외의 언어를 사용한다면 OpenAI보다 더 나은 선택입니다.",
    ...
    "triggerDesc": {
        ...
        "v2ImgGen": "이미지 생성",
        "v2ImgGenDesc": "{{value}} 프롬프트, {{negValue}} 네거티브 프롬프트로 이미지 생성 => {{outputVar}}",
        ...
        "v2SpliceArrayVarDesc": "배열 변수 {{var}} 의 {{start}}에 {{value}} 추가",
    ...
    "hub": "RisuRealm",
    "hubLoadFailed": "RisuRealm을 불러오지 못했습니다. 연결 상태를 확인한 후 다시 시도해 주세요.",
    "hubEmpty": "현재 RisuRealm에 표시할 캐릭터가 없습니다.",
    "hubOffline": "오프라인 상태입니다. 인터넷에 연결하면 RisuRealm을 둘러볼 수 있습니다.",
    "hubRetry": "재시도",
    "hubBrowseMore": "RisuRealm 둘러보기",
    "hubDescription": "RisuRealm에서 더 많은 캐릭터를 둘러보세요.",
    "hubAnnouncementTitle": "RisuRealm에서 온 공지",
    "homeVersion": "버전",
    "homeLinkDiscordTitle": "디스코드",
    "homeLinkDiscordDescription": "디스코드 서버에 참여해 다른 사용자, 개발자와 대화해 보세요.",
    "homeLinkWebsiteTitle": "웹사이트",
    "homeLinkWebsiteDescription": "프로젝트의 공식 웹사이트를 확인해 보세요.",
    "homeLinkGithubTitle": "소스 코드 및 이슈",
    "homeLinkGithubDescription": "GitHub에서 소스 코드를 둘러보거나 이슈를 등록해 보세요.",
    "homeSourceRepoLabel": "소스 코드",
    "homeSourceIssuesLabel": "이슈",
    "homeSourceUpstreamLabel": "업스트림",
    "homeLinkEmailTitle": "이메일",
    "homeLinkEmailDescription": "개발자에게 직접 연락하세요.",
    "homeSourceEmailForkLabel": "메인테이너에게 문의",
    ...
    "duplicateChaIdSavePausedMessage": (characterGroups: string) =>
        `다음 캐릭터가 다른 캐릭터와 내부 ID를 공유하고 있어, 저장 파일은 공유된 ID마다 하나만 유지할 수 있습니다: ${characterGroups || "캐릭터 두 개"}. 이 문제가 시작되기 전에 가장 최근에 저장되었던 캐릭터를 유지하며, 둘 다 저장된 적이 없다면 먼저 기록된 캐릭터를 유지합니다. 이 상태가 계속되는 동안에는 이들 모두에 대한 저장이 중단됩니다. 각 ID를 하나의 캐릭터만 갖게 되면 저장이 재개됩니다 — 예를 들어 나머지 캐릭터를 영구적으로 삭제하면 됩니다 (휴지통으로 이동한 뒤 휴지통에서 삭제하세요).`,
    ...
    "trimStartNewChat": "'새 채팅 시작' 메시지 잘라내기",
    "pluginDevelopMode": "플러그인 개발 모드",
    "fetchLogConsent": "플러그인 {}이(가) 로그를 가져오려고 합니다. 이는 민감한 정보를 노출할 수 있습니다. 허용하시겠습니까?",
    "getFullDatabaseConsent": "플러그인 {}이(가) 전체 데이터베이스에 접근하려고 합니다. 이는 민감한 정보를 노출할 수 있습니다. 허용하시겠습니까?",
    "mainDomAccessConsent": "플러그인 {}이(가) 메인 Document에 접근하려고 합니다. 이는 민감한 정보를 노출할 수 있습니다. 허용하시겠습니까?",
    "replacerPermissionConsent": "플러그인 {}이(가) 채팅 내용을 대체할 수 있는 권한을 요청하고 있습니다. 이는 대화 내용을 조작하는 데 사용될 수 있습니다. 허용하시겠습니까?",
    "providerPermissionConsent": "플러그인 {}이(가) 제공자에 접근할 수 있는 권한을 요청하고 있습니다. 이는 무단 API 호출을 가능하게 할 수 있습니다. 허용하시겠습니까?",
    "inlayPermissionConsent": "플러그인 {}이(가) 인레이에 접근할 수 있는 권한을 요청하고 있습니다. 이는 인레이 내용을 읽고, 쓰고, 수정할 수 있게 합니다. 허용하시겠습니까?",
    "sendChatConsent": "플러그인 {}이(가) 사용자를 대신하여 채팅 메시지를 보낼 수 있는 권한을 요청하고 있습니다. 이는 AI 응답을 트리거합니다. 허용하시겠습니까?",
    "pluginV2Warning": "플러그인 V2 및 V2.1은 안전하지 않은 것으로 간주되며 향후 버전에서 작동이 중단됩니다. **이 버전의 플러그인은 사용하지 마세요.** 만약 이 플러그인의 개발자라면, 가능한 한 빨리 V3로 업데이트해주세요.",
    ...
    "encryptedBackupRefused": "이 백업은 RisuAccount로 암호화되어 있어 기술적인 이유로 여기서 읽을 수 없습니다. 아무것도 가져오지 않았습니다.\n\n업스트림 RisuAI에서는 이 데이터를 여전히 옮길 수 있습니다:\n\n1. \"부분 로컬 백업 저장 (캐릭터 에셋 제외)\"을 사용하세요. 이 백업은 암호화되지 않으며 콜드 스토리지를 포함한 모든 채팅을 보존합니다. 캐릭터, 그룹, 페르소나 프로필 이미지, 사용자 아이콘, 배경, 폴더 및 프리셋 이미지도 유지되며, 그 외의 이미지와 VITS 파일은 제외됩니다.\n\n2. 계정 동기화에서 \"로그아웃\"한 다음 \"로컬 백업 저장\"을 사용하세요. 이 방법은 .png 에셋은 유지하지만 콜드 스토리지 채팅은 잃게 됩니다.\n\n두 방법을 모두 수행하고 여기서 둘 다 가져오면 모든 것을 보존할 수 있습니다. 자세한 내용은 마이그레이션 가이드를 참조하세요.",
    "encryptedBackupImportStopped": "이 백업에 암호화된 부분이 포함되어 있어 가져오기가 중단되었습니다. 일부 이미지나 콜드 스토리지 항목이 이미 추가되거나 교체되었을 수 있습니다. 현재 데이터베이스는 변경되지 않았습니다.",
    "backupFileUnreadable": "파일을 읽을 수 없습니다. 아무것도 가져오지 않았습니다.",
    ...
    "blockquoteStyling": "인용구 스타일링",
    "dynamicModelRegistry": "동적 모델 레지스트리",
    "confirmInstallPluginViaPlugin": "다른 플러그인을 통해 플러그인을 설치하려고 합니다. 출처가 되는 플러그인을 신뢰할 수 없다면 위험할 수 있습니다. 계속하시겠습니까?",
    "risuaiProTools": "Risuai Pro Tools",
    "enableRisuaiProTools": "Risuai Pro Tools 활성화",
    "easyPanel": "이지 패널",
    "mainModel": "메인 모델",
    "epEnabled": "이지 패널 오버라이드",
    "epRequirementsNotMet": "이지 패널 요구사항이 충족되지 않았습니다. 아래 버튼을 클릭하면 이지 패널에 최적화된 설정으로 자동 조정됩니다.",
    "seperateParametersByModel": "모델별로 매개변수 분리",
    "disableSeperateParameterChangeOnPresetChange": "프리셋 변경 시 매개변수 분리 변경 비활성화",
    "keepSessionAlive": "세션 유지",
    "keepSessionAlivePip": "PIP를 통해",
    "keepSessionAliveSound": "소리를 통해",
    ...
    realmNewListingConfirm: "이 작업은 새로운 RisuRealm 게시물을 생성합니다. 기존 게시물을 수정하려면 RisuRealm 사이트에서 직접 로그인하세요. 계속하시겠습니까?",
    upstreamAgreementPromptBefore: "이 기능은 이 앱이 아닌 업스트림 RisuAI가 운영하는 서비스를 사용합니다. 계속하려면 다음 약관에 동의해야 합니다: ",
    upstreamAgreementTermsOfService: "이용약관",
    upstreamAgreementPromptBetween: " 및 ",
    upstreamAgreementPrivacyPolicy: "개인정보 처리방침",
    upstreamAgreementPromptAfter: ".",
    upstreamAgreementAccept: "동의",
    upstreamAgreementDecline: "동의하지 않음",
    upstreamConsentPlaceholder: "Realm 콘텐츠는 업스트림 RisuAI가 운영하는 서비스에서 제공됩니다. 표시하려면 약관에 동의하세요.",
    upstreamConsentShow: "동의하고 보기",
