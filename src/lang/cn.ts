export const languageChinese = {
    ...
    "errors": {
        ...
        "networkFetchWeb": "这可能是由 CORS 错误引起的。这种情况只会在使用网页版时发生，因为浏览器存在一些限制。请尝试使用本地版或其他版本的 Risuai。",
        ...
        "coldStorageChatDataMissing": (coldDataKey: string) =>
            `此聊天的存储数据未找到（键：${coldDataKey}）。如果您在其他设备上有备份，请先恢复该备份。否则数据已丢失，您可以删除此聊天。`,
    ...
    "help": {
        ...
        "localNetworkModeDesc": "将私有/局域网模型 URL 通过本地运行时路径转发，而非使用浏览器直接抓取。\n\n**用途**\n- 避免浏览器对 `192.168.x.x`、`10.x.x.x`、`localhost`、`.local` 等本地主机的私有网络/CORS 限制。\n- 降低 Node 自托管模式下本地推理首个 Token 较慢时的超时风险。\n\n**工作原理**\n- 仅在启用本地网络模式且目标 URL 被识别为本地/私有地址时生效。\n- Node 自托管：流式请求优先使用实验性的 Job+WebSocket 中继（失败时回退到 `/proxy2`）；非流式请求使用 `/proxy2`。\n- Tauri：使用原生/直连路径。\n- 公共网页模式：出于设计考虑，将阻止本地/私有直连请求。\n\n**限制**\n- 仅适用于兼容 OpenAI 的请求路径。\n- 此功能不会绕过两个公共域名之间的 Cloudflare 源限制。\n- 需使用您的自托管 URL（即 `globalThis.__NODE__ === true` 的环境）才能生效。",
        ...
        "chatLoadInitialPages": "打开聊天界面时初始渲染的最近聊天消息数量。数值越高，能立即显示更多历史，但打开长聊天时可能更卡。",
        "chatLoadAdditionalPages": "每次向上滚动到顶部时额外渲染的旧聊天消息数量。数值越高，重复加载次数越少，但每次加载可能更重。",
        ...
        "antiClaudeOverload": "若 Claude 过载发生，Risuai 会透过继续相同的提示来阻止它，以减少过载的概率。此功能仅适用于串流回应（Streamed Responses），对非官方 API 端点可能无效。",
        ...
        "defaultVariables": "可在此处设置自定义默认变量。使用 `<variable name>=<variable value>` 格式，每行一个。例如：`name=Risuai`，可在触发式和 CBS 变量中使用，如：`{{getvar::A}}`、`{{setvar::A::B}}` 或 `{{? $A + 1}}`。若提示词模板的默认变量与角色的默认变量名称相同，系统将使用角色的默认变量。",
        ...
        "hypaV3MaxChatsPerSummary": "创建单个总结时包含的最大聊天消息数量。",
        "hypaV3RecentMemoryRatio": "分配给最近记忆的记忆 Token 比例。系统会自动用最新创建的总结填充，直到分配的 Token 用完为止。",
        "hypaV3SimilarMemoryRatio": "分配给相似记忆的记忆 Token 比例。系统会自动用与最近聊天相似度得分最高的总结填充，直到分配的 Token 用完为止。",
        "hypaV3RandomMemoryRatio": "从尚未被其他类别选中的总结中随机填充。",
        "hypaV3PreserveOrphanedMemory": "如果启用，引用已删除聊天消息的总结将被保留。如果禁用，源消息已不存在的总结将被自动移除。",
        "hypaV3ProcessRegexScript": "如果启用，在 HypaV3 弹窗中重新生成总结时，将对输入的聊天消息应用正则脚本。",
        "hypaV3DoNotSummarizeUserMessage": "如果启用，用户消息将不计入每个总结的最大消息数中。",
        "hypaV3EnableSimilarityCorrection": "如果启用，最近聊天的总结也将额外用作查询。此选项与实验性的 HypaMemory V3 不兼容。",
        "hypaV3UseExperimentalImpl": "切换到实验性的 HypaMemory V3 实现。启用速率限制设置并更改查询方式。",
        "hypaV3AlwaysToggleOn": "如果启用，选择角色时将自动激活 HypaMemory 开关。",
        "hypaV3SummarizationRequestsPerMinute": "用于总结的 SupaMemory 模型每分钟最大请求数。仅当 SupaMemory 模型设置为辅助模型时适用。",
        "hypaV3SummarizationMaxConcurrent": "用于总结的 SupaMemory 模型最大并发请求数。仅当 SupaMemory 模型设置为辅助模型时适用。",
        "hypaV3EmbeddingRequestsPerMinute": "用于相似度搜索的嵌入模型每分钟最大请求数。",
    ...
    "setup": {
        ...
        "claudeDesc": "Claude 是由 Anthropic 开发的 AI 模型，是 OpenAI 的竞争对手。如果你希望使用非英语语言，它优于 GPT。",
    ...
    "chatLoadInitialPages": "聊天初始加载数量",
    "chatLoadAdditionalPages": "聊天追加加载数量",
    ...
    "remindLaterQuestion": "什么时候再提醒您？",
    "remindIgnore": "忽略",
    "remindLater1Day": "1天后提醒",
    "remindLater3Days": "3天后提醒",
    "remindLater5Days": "5天后提醒",
    "remindLater1Week": "1周后提醒",
    "display": "显示 & 音频",
    ...
    "hub": "RisuRealm",
    "hubLoadFailed": "加载 RisuRealm 失败，请检查网络连接后重试。",
    "hubEmpty": "RisuRealm 目前没有找到任何角色。",
    "hubOffline": "您当前处于离线状态。连接到互联网后即可浏览 RisuRealm。",
    "hubRetry": "重试",
    "hubBrowseMore": "浏览 RisuRealm",
    "hubDescription": "使用 RisuRealm 浏览更多角色。",
    "hubAnnouncementTitle": "来自 RisuRealm 的公告",
    "homeVersion": "版本",
    "homeLinkDiscordTitle": "Discord",
    "homeLinkDiscordDescription": "加入我们的 Discord 服务器，与其他用户和开发者交流。",
    "homeLinkWebsiteTitle": "网站",
    "homeLinkWebsiteDescription": "查看项目的官方网站。",
    "homeLinkGithubTitle": "源代码 & 议题",
    "homeLinkGithubDescription": "浏览源代码，或在 GitHub 上提交议题。",
    "homeSourceRepoLabel": "源代码",
    "homeSourceIssuesLabel": "议题",
    "homeSourceUpstreamLabel": "上游",
    "homeLinkEmailTitle": "邮箱",
    "homeLinkEmailDescription": "直接联系开发者。",
    "homeSourceEmailForkLabel": "联系维护者",
    ...
    "officialWikiDesc": "欢迎查看 Risuai 官方的 Wiki。",
    "officialDiscord": "官方 Discord",
    "officialDiscordDesc": "Risuai 官方的 Discord 服务器",
    "persona": "人设",
    "icon": "头像",
    "remove": "删除",
    "able": "激活",
    ...
    "donatorPatreonDesc": "为保护隐私，默认不会显示在名单中。若想显示您的昵称，请前往 Risuai 的 Patreon 页面并点击链接按钮。",
    ...
    "openRouterFallback": "使用回退",
    "openRouterMiddleOut": "使用中间输出",
    "openRouterProviderOrder": "提供商优先顺序",
    "openRouterProviderOnly": "允许的提供商",
    "openRouterProviderIgnore": "忽略的提供商",
    "openRouterSearchModel": "搜索模型...",
    "openRouterSortByName": "按名称",
    "openRouterSortByPrice": "按价格",
    "openRouterSortByProvider": "按厂商",
    ...
    "shareExport": "分享/导出",
    "risupresetDesc": "Risupreset 是专为 Risuai 默认设置设计的格式。",
    "risuMDesc": "RisuM 是专为 Risuai 模块设计的格式。",
    ...
    "includePersonaName": "包含人设名称",
    "hidePersonaName": "隐藏人设名称",
    ...
    "bindPersona": "绑定人设",
    "chatOptions": "聊天选项",
    "doYouWantToBindCurrentPersona": "是否将当前人设绑定至此聊天？",
    "doYouWantToUnbindCurrentPersona": "是否解除此对话中的人设绑定？",
    "personaBindedSuccess": "人设绑定已完成",
    "personaUnbindedSuccess": "人设绑定已解除",
    "parameters": "参数",
    "sizeAndSpeed": "大小与速度",
    "useLegacyGUI": "使用旧版接口",
    ...
    clearTranslationCacheConfirm: "这将删除所有翻译缓存条目，此操作不可撤销。是否继续？",
    "loading": "加载中",
    ...
    "escapeOutput": "转义输出",
    "openAIFlexProcessing": "OpenAI Flex 处理",
    ...
    "bootstrap": {
        ...
        "reportErrorQuestion": "是否要将此错误报告给开发者？\n这有助于防止同样的错误再次发生。\n（仅包含损坏的模块信息，不含个人数据）",
        "diagnosticInformation": (reportData: string) => `# 诊断信息\n\n请复制以下信息，并通过 GitHub Issues、Discord 或社区将其报告给开发者。\n报告前请仔细检查是否包含任何敏感信息。\n\n\`\`\`json\n${reportData}\n\`\`\``,
        "resetLorebookQuestion": "是否仅重置该特定损坏模块中的世界书数据？\n（其他模块和角色数据不会受到影响）\n\n选择\"否\"将保留数据，但错误可能会持续出现。"
    },
    "partialEdit": {
        "editButtonTooltip": "编辑",
        "deleteButtonTooltip": "删除",
        "deleteModalTitle": "删除确认",
        "deleteConfirmMessage": "确定要删除这部分内容吗？",
        "deleteYes": "是",
        "deleteNo": "否",
        "editModalTitle": "局部编辑",
        "matchFound": (method: string) => `匹配方式：${method}`,
        "matchFailedTitle": "无法进行局部编辑",
        "matchFailedMessage": "无法在原始消息中精确定位。消息过长或存在重复内容时可能出现此问题。请改用完整消息编辑功能。",
    ...
    "confirmInstallPluginViaPlugin": "您正在通过另一个插件安装新插件。如果来源插件不可信，这可能带来风险。是否继续？",
    "risuaiProTools": "Risuai Pro Tools",
    "enableRisuaiProTools": "启用 Risuai Pro Tools",
    "easyPanel": "Easy Panel",
    "mainModel": "主模型",
    "epEnabled": "Easy Panel 覆盖",
    "epRequirementsNotMet": "未满足 Easy Panel 的使用条件。点击下方按钮可自动调整为最佳的 Easy Panel 设置。",
    "seperateParametersByModel": "按模型分离参数",
    "disableSeperateParameterChangeOnPresetChange": "切换预设时不更改分离参数",
    "keepSessionAlive": "保持会话活跃",
    "keepSessionAlivePip": "通过画中画（PIP）",
    "keepSessionAliveSound": "通过声音",
    "nanoGPTLoadingAccountInfo": "正在加载账户信息…",
    "nanoGPTCreditBalance": "余额:",
    "nanoGPTSubscription": "订阅",
    "nanoGPTGraceUntil": (date: string) => `至 ${date}`,
    "nanoGPTNoActiveSubscription": "无活跃订阅。",
    "nanoGPTCancelsAtPeriodEnd": (date: string) => `到期后取消 (${date})`,
    "nanoGPTWeeklyTokens": (pct: string) => `每周包含输入 Token — 已使用 ${pct}`,
    "nanoGPTDailyTokens": (pct: string) => `每日包含输入 Token — 已使用 ${pct}`,
    "nanoGPTDailyImages": (pct: string) => `每日包含图片 — 已使用 ${pct}`,
    "nanoGPTResets": (date: string) => `重置于 ${date}`,
    "nanoGPTUsed": (n: string) => `已使用 ${n}`,
    "nanoGPTRemaining": (n: string) => `剩余 ${n}`,
    "nanoGPTRenews": (date: string) => `续费日期: ${date}`,
    "nanoGPTCouldNotLoadAccountInfo": "无法加载账户信息。",
    "nanoGPTWeeklyTokensLabel": "每周包含输入令牌",
    "nanoGPTDailyTokensLabel": "每日包含输入令牌",
    "nanoGPTDailyImagesLabel": "每日包含图片",
    "nanoGPTUsedLabel": "已使用",
    "nanoGPTRemainingLabel": "剩余",
    "nanoGPTResetsLabel": "重置",
    "nanoGPTRenewsLabel": "续订:",
    "nanoGPTProvider": "服务商",
    "nanoGPTProviderPayAsYouGoOnly": "(仅在按量付费模式下可用)",
    "nanoGPTProviderAuto": "自动",
    "nanoGPTProviderFree": "免费",
    "nanoGPTProviderInput": "输入",
    "nanoGPTProviderOutput": "输出",
    "nanoGPTProviderQuantization": "量化",
    "nanoGPTProviderUndisclosed": "未公开",
    "nanoGPTProviderCache": "缓存",
    "nanoGPTProviderCacheSupported": "支持",
    "nanoGPTProviderCacheNotSupported": "不支持",
    "nanoGPTProviderCacheRead": "缓存读取",
    "nanoGPTUseSubscriptionEndpoint": "使用订阅端点和模型",
    ...
    "realmNewListingConfirm": "此操作将创建一个新的 RisuRealm 条目。如果要编辑现有条目，请改在 RisuRealm 官方网站登录。是否要继续？",
    "upstreamAgreementPromptBefore": "此功能使用的服务由上游 RisuAI 运营，而非本应用。若要继续，请同意其",
    "upstreamAgreementTermsOfService": "服务条款",
    "upstreamAgreementPromptBetween": "和",
    "upstreamAgreementPrivacyPolicy": "隐私政策",
    "upstreamAgreementPromptAfter": "。",
    "upstreamAgreementAccept": "同意",
    "upstreamAgreementDecline": "不同意",
    "upstreamConsentPlaceholder": "Realm 内容来自上游 RisuAI 运营的服务。请同意其条款以显示。",
    "upstreamConsentShow": "同意并显示",
