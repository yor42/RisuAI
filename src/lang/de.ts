export const languageGerman = {
    ...
    "errors": {
        ...
        "networkFetchWeb": "Dies kann ein CORS-Fehler sein. Dies geschieht nur bei Verwendung der Webversion aufgrund von Browserbeschränkungen. Versuchen Sie, die lokale Desktop-Version oder eine andere Version von Risuai zu verwenden.",
        ...
        "coldStorageChatLoadFailed": (coldDataKey: string) =>
            `Die gespeicherten Daten dieses Chats konnten nicht geladen werden (Schlüssel: ${coldDataKey}). Dies kann ein vorübergehendes Problem sein — versuchen Sie, den Chat zu wechseln oder neu zu starten. Wenn es weiterhin fehlschlägt, sind die Daten möglicherweise verloren und Sie können diesen Chat löschen. Führen Sie bis dahin nicht die Funktion „Kaltlager bereinigen" aus.`,
        "coldStorageChatDataMissing": (coldDataKey: string) =>
            `Die gespeicherten Daten dieses Chats konnten nicht gefunden werden (Schlüssel: ${coldDataKey}). Wenn Sie eine Sicherung von einem anderen Gerät haben, stellen Sie diese zuerst wieder her. Andernfalls sind die Daten verloren und Sie können diesen Chat löschen.`,
    ...
    "help": {
        ...
        "localNetworkModeDesc": "Leitet private/LAN-Modell-URLs über den lokalen Laufzeitpfad um, anstatt den direkten Browser-Fetch zu verwenden.\n\n**Zweck**\n- Vermeidet Browser-Einschränkungen für private Netzwerke/CORS bei `192.168.x.x`, `10.x.x.x`, `localhost`, `.local` und ähnlichen lokalen Hosts.\n- Verringert das Timeout-Risiko bei langsamer erster Token-Generierung durch lokale Inferenz im Node-Self-Host-Modus.\n\n**Funktionsweise**\n- Gilt nur, wenn der lokale Netzwerkmodus aktiviert ist und die Ziel-URL als lokal/privat erkannt wird.\n- Node-Self-Host: Beim Streaming wird zunächst das experimentelle Job+WebSocket-Relay verwendet (bei Fehlschlag Fallback auf `/proxy2`); ohne Streaming wird `/proxy2` verwendet.\n- Tauri: verwendet den nativen/direkten Pfad.\n- Öffentlicher Web-Modus: direkte Aufrufe an lokale/private Adressen sind hier bewusst blockiert.\n\n**Einschränkungen**\n- Der Geltungsbereich umfasst nur OpenAI-kompatible Anfragepfade.\n- Dies umgeht keine Cloudflare-Origin-Beschränkungen zwischen zwei öffentlichen Domains.\n- Verwenden Sie Ihre Self-Host-URL (bei der `globalThis.__NODE__ === true` gilt), damit diese Funktion wirkt.",
        "chainOfThought": "Wenn aktiviert, wird der Anweisung eine 'Chain of Thought'-Anweisung hinzugefügt.",
        "gptVisionQuality": "Diese Option wird verwendet, um die Qualität des Bilderkennungsmodells festzulegen. Je höher die Qualität, desto genauer die Erkennung, aber es werden mehr Tokens verwendet.",
        "genTimes": "Diese Option wird verwendet, um die Anzahl der Antworten festzulegen, die auf Unterstützungsmodellen generiert werden sollen. Andere als die erste Antwort fungieren als zwischengespeicherter Reroll. Dies kann die Kosten des Modells senken, aber auch erhöhen, wenn Sie es ohne Reroll verwenden.",
        ...
        "chatLoadInitialPages": "Anzahl der neuesten Chatnachrichten, die beim Öffnen eines Chats gerendert werden. Höhere Werte zeigen sofort mehr Verlauf, können lange Chats aber schwerer öffnen lassen.",
        "chatLoadAdditionalPages": "Anzahl älterer Chatnachrichten, die jedes Mal zusätzlich gerendert werden, wenn Sie nach oben scrollen. Höhere Werte reduzieren wiederholtes Laden, können aber jeden Ladevorgang schwerer machen.",
        "emotionPrompt": "Diese Option wird verwendet, um die Anweisung festzulegen, die zur Erkennung von Emotionen verwendet wird. Wenn sie leer ist, wird die Standardanweisung verwendet.",
        "removePunctuationHypa": "Wenn aktiviert, werden Satzzeichen entfernt, bevor HypaMemory ausgeführt wird.",
        "additionalParams": "Zusätzliche Parameter, die dem Anfrage-Body hinzugefügt werden. Wenn Sie einige Parameter ausschließen möchten, können Sie `{{none}}` als Wert eingeben. Wenn Sie einen Header anstelle eines Body hinzufügen möchten, können Sie `header::` vor den Schlüssel setzen, z. B. `header::Authorization`. Wenn Sie den Wert als JSON wünschen, können Sie `json::` vor den Wert setzen, z. B. `json::{\"key\":\"value\"}`. Andernfalls wird der Typ des Wertes automatisch bestimmt.",
        "antiClaudeOverload": "Wenn eine Claude-Überlastung auftritt, versucht Risuai, diese zu verhindern, indem es mit derselben Anweisung fortfährt, wodurch die Wahrscheinlichkeit verringert wird. Funktioniert nur für gestreamte Antworten. Dies funktioniert möglicherweise nicht für inoffizielle API-Endpunkte.",
        "triggerScript": "Trigger Script ist ein benutzerdefiniertes Skript, das ausgeführt wird, wenn eine Bedingung erfüllt ist. Es kann verwendet werden, um die Chat-Daten zu ändern, einen Befehl auszuführen, eine Variable zu ändern usw. Der Typ hängt davon ab, wann es ausgelöst wird. Es kann auch durch Schaltflächen ausgeführt werden, die mit {{button::Display::TriggerName}} oder HTML-Schaltflächen mit dem Attribut `risu-trigger=\"<TriggerName>\"` verwendet werden können.",
        "autoContinueChat": "Wenn aktiviert, wird versucht, den Chat fortzusetzen, wenn er nicht mit einem Satzzeichen endet. VERWENDEN SIE DIES NICHT BEI SPRACHEN, DIE KEINE SATZZEICHEN VERWENDEN.",
        "combineTranslation": "Wenn aktiviert, wird Text, der einen Satz bildet, aber durch HTML-Tags getrennt ist, zusammengefasst und übersetzt. Anschließend wird das Skript 'Modify Display' erneut auf die übersetzte Ausgabe angewendet.\nDies hilft dem Übersetzer, eine korrekte Übersetzung zu erstellen.\nWenn die Benutzeroberfläche bei Aktivierung dieser Option seltsam aussieht, deaktivieren Sie die Option bitte und melden Sie dies.",
        "dynamicAssets": "Wenn aktiviert, wird versucht, den ähnlichsten Asset-Namen mithilfe der Vektorsuche zu finden und durch diesen zu ersetzen, falls der Asset-Name bei der Datenverarbeitung nicht gefunden wird.",
        ...
        "v2Calculate": "Wertet mathematische Ausdrücke aus und unterstützt Grundrechenarten (+, -, *, /, %, ^), Vergleichsoperatoren (<, >, <=, >=, =, !=), logische Operatoren (&&, ||, !), Klammern für die Rangfolge und Variablensubstitution im Format $VariablenName. Variablen werden automatisch in Zahlen umgewandelt (Standard ist 0, wenn ungültig).",
        "namespace": "Der Namespace ist eine eindeutige Kennung für das Modul. Er wird verwendet, um Konflikte zwischen Modulen zu vermeiden und für die Interaktion von Voreinstellungen, anderen Modulen usw. Wenn Sie nicht sicher sind, was Sie eingeben sollen, lassen Sie es leer.",
        "moduleIntergration": "Sie können Module aktivieren, indem Sie den Modul-Namespace in die Modul-Integrationsabschnitte eingeben. Wenn Sie mehrere Module aktivieren möchten, können Sie diese durch Kommas trennen. Zum Beispiel `Modul1,Modul2,Modul3`. Dies ist für fortgeschrittene Benutzer gedacht, die die Verwendung von Modulen je nach Voreinstellung variieren möchten.",
        ...
        "summarizationPrompt": "Die Anweisung, die für die Zusammenfassung verwendet wird. Wenn sie leer ist, wird die Standardanweisung verwendet. Sie können auch ChatML-Formatierung mit {{slot}} für die Chat-Daten verwenden.",
        "translatorPrompt": "Die Anweisung, die für die Übersetzung verwendet wird. Wenn sie leer ist, wird die Standardanweisung verwendet. Sie können auch ChatML-Formatierung mit {{slot}} für die Zielsprache, {{slot::content}} für den Inhalt und {{slot::tnote}} für die Übersetzungsnotiz verwenden.",
        "translateBeforeHTMLFormatting": "Wenn aktiviert, wird der Text vor Regex-Skripten und HTML-Formatierung übersetzt. Dies könnte die Token verringern, aber die Formatierung beeinträchtigen.",
        "autoTranslateCachedOnly": "Wenn zusammen mit der Option 'Automatische Übersetzung' aktiviert, werden automatisch nur die Nachrichten übersetzt, die der Benutzer zuvor übersetzt hat.",
        "presetChain": "Wenn es nicht leer ist, wird die Voreinstellung geändert und jedes Mal zufällig angewendet, wenn der Benutzer eine Nachricht in der Voreinstellungsliste in dieser Eingabe sendet. Die Voreinstellungsliste sollte durch Kommas getrennt werden, zum Beispiel `preset1,preset2`.",
        ...
        "keepSessionAlive": "Hält den Tab aktiv und verhindert, dass die Sitzung in Browsern wegen Inaktivität abläuft. Damit dies wirksam wird, ist möglicherweise eine Aktualisierung erforderlich.\n\n- **Über Ton**: Spielt in regelmäßigen Abständen ein stummes Audio ab, um die Sitzung aktiv zu halten. Diese Methode gilt in den meisten Browsern als am kompatibelsten und effektivsten.\n",
        "reSummarizationPrompt": "Die Anweisung, die beim Zusammenführen mehrerer ausgewählter Zusammenfassungen zu einer einzigen per Sammelbearbeitung verwendet wird. Wenn sie leer ist, wird die Standardanweisung verwendet.",
        "hypaV3MemoryTokensRatio": "Der Anteil der maximalen Kontextgröße, der dem Langzeitgedächtnis-Block {{slot}} in der Anweisung zugewiesen wird.",
        "hypaV3ExtraSummarizationRatio": "Senkt den Schwellenwert, bei dem die Zusammenfassung stoppt. Bei 0 stoppt die Zusammenfassung, sobald die Tokenanzahl unter die maximale Kontextgröße fällt. Höhere Werte führen dazu, dass vor dem Stopp mehr zusammengefasst wird.",
        "hypaV3MaxChatsPerSummary": "Maximale Anzahl an Chat-Nachrichten, die beim Erstellen einer einzelnen Zusammenfassung einbezogen werden.",
        "hypaV3RecentMemoryRatio": "Der Anteil der Gedächtnis-Tokens, der dem aktuellen Gedächtnis zugewiesen wird. Wird automatisch mit den zuletzt erstellten Zusammenfassungen aufgefüllt, bis die zugewiesenen Tokens ausgeschöpft sind.",
        "hypaV3SimilarMemoryRatio": "Der Anteil der Gedächtnis-Tokens, der dem ähnlichen Gedächtnis zugewiesen wird. Wird automatisch mit den Zusammenfassungen aufgefüllt, die die höchste Ähnlichkeit mit den letzten Chats aufweisen, bis die zugewiesenen Tokens ausgeschöpft sind.",
        "hypaV3RandomMemoryRatio": "Wird zufällig mit Zusammenfassungen aufgefüllt, die noch nicht von anderen Kategorien ausgewählt wurden.",
        "hypaV3PreserveOrphanedMemory": "Wenn aktiviert, werden Zusammenfassungen, die sich auf gelöschte Chat-Nachrichten beziehen, beibehalten. Wenn deaktiviert, werden Zusammenfassungen, deren Quellnachrichten nicht mehr existieren, automatisch entfernt.",
        "hypaV3ProcessRegexScript": "Wenn aktiviert, werden Regex-Skripte auf die eingehenden Chat-Nachrichten angewendet, wenn Zusammenfassungen im HypaV3-Dialog neu erstellt werden.",
        "hypaV3DoNotSummarizeUserMessage": "Wenn aktiviert, werden Benutzernachrichten von der maximalen Anzahl an Nachrichten pro Zusammenfassung ausgeschlossen.",
        "hypaV3EnableSimilarityCorrection": "Wenn aktiviert, wird zusätzlich eine Zusammenfassung der letzten Chats als Abfrage verwendet. Funktioniert nicht mit der experimentellen HypaMemory V3.",
        "hypaV3UseExperimentalImpl": "Wechselt zur experimentellen HypaMemory-V3-Implementierung. Aktiviert Ratenlimit-Einstellungen und ändert die Abfragemethode.",
        "hypaV3AlwaysToggleOn": "Wenn aktiviert, wird der HypaMemory-Schalter automatisch aktiviert, sobald ein Charakter ausgewählt wird.",
        "hypaV3SummarizationRequestsPerMinute": "Maximale Anzahl an SupaMemory-Modellanfragen pro Minute für die Zusammenfassung. Gilt nur, wenn das SupaMemory-Modell auf Hilfsmodell eingestellt ist.",
        "hypaV3SummarizationMaxConcurrent": "Maximale Anzahl gleichzeitiger SupaMemory-Modellanfragen für die Zusammenfassung. Gilt nur, wenn das SupaMemory-Modell auf Hilfsmodell eingestellt ist.",
        "hypaV3EmbeddingRequestsPerMinute": "Maximale Anzahl an Embedding-Modellanfragen pro Minute für die Ähnlichkeitssuche.",
    ...
    "chatLoadInitialPages": "Anfängliche Chat-Ladeanzahl",
    "chatLoadAdditionalPages": "Zusätzliche Chat-Ladeanzahl",
    ...
    "enableBlockPartialEdit": "Blockweise Teilbearbeitung aktivieren (Zum Bearbeiten einzelner Blöcke darüberfahren)",
    "enableDragPartialEdit": "Teilbearbeitung per Ziehen aktivieren (Text zum Bearbeiten auswählen)",
    ...
    "hub": "RisuRealm",
    "hubLoadFailed": "RisuRealm konnte nicht geladen werden. Bitte überprüfen Sie Ihre Verbindung und versuchen Sie es erneut.",
    "hubEmpty": "Derzeit wurden keine Charaktere in RisuRealm gefunden.",
    "hubOffline": "Sie sind offline. Verbinden Sie sich mit dem Internet, um RisuRealm zu durchsuchen.",
    "hubRetry": "Wiederholen",
    "hubBrowseMore": "RisuRealm durchsuchen",
    "hubDescription": "Entdecken Sie weitere Charaktere über RisuRealm.",
    "hubAnnouncementTitle": "Ankündigung von RisuRealm",
    "homeVersion": "Version",
    "homeLinkDiscordTitle": "Discord",
    "homeLinkDiscordDescription": "Treten Sie unserem Discord-Server bei, um mit anderen Nutzern und dem Entwickler zu chatten.",
    "homeLinkWebsiteTitle": "Website",
    "homeLinkWebsiteDescription": "Besuchen Sie die offizielle Website des Projekts.",
    "homeLinkGithubTitle": "Quellcode & Issues",
    "homeLinkGithubDescription": "Sehen Sie sich den Quellcode an oder erstellen Sie ein Issue auf GitHub.",
    "homeSourceRepoLabel": "Quellcode",
    "homeSourceIssuesLabel": "Issues",
    "homeSourceUpstreamLabel": "Upstream",
    "homeLinkEmailTitle": "E-Mail",
    "homeLinkEmailDescription": "Kontaktieren Sie den Entwickler direkt.",
    "homeSourceEmailForkLabel": "Maintainer kontaktieren",
    ...
    "draftRestored": "Nicht gespeicherte Bearbeitung wiederhergestellt",
    "draftRevert": "Verwerfen",
    ...
    "hypaV3Settings": {
        ...
        "queryChatCountLabel": "Anzahl der Abfrage-Chats",
        "summaryChunkSeparatorLabel": "Trennzeichen-Regex für Abschnitte",
    ...
    "thinkingType": "Denkmodus",
    "adaptiveThinkingEffort": "Adaptiver Denkaufwand",
    ...
    "openAIFlexProcessing": "OpenAI Flex-Verarbeitung",
    ...
    "pluginRiskDetectedAlert": "Risuai hat Code erkannt, der schädlich sein könnte und IHREN API-SCHLÜSSEL ODER PERSÖNLICHE DATEN STEHLEN KANN. Wir empfehlen Ihnen, DIESES PLUGIN NICHT ZU INSTALLIEREN.",
    "doNotInstall": "Nicht installieren",
    "continueAnyway": "Trotzdem fortfahren",
    "pluginNoRiskButAlert": "Risuai hat keinen schädlichen Code erkannt, aber beachten Sie bitte, dass die Installation von Plugins aus nicht vertrauenswürdigen Quellen Sicherheitsrisiken bergen kann.",
    ...
    "createFolderOnBranch": "Ordner beim Verzweigen erstellen",
    "partialBackupFirstConfirm": "⚠️ WARNUNG: Teilweise lokale Sicherung ⚠️\n\nDiese Sicherung speichert schnell nur die wichtigsten Informationen.\n\nEnthalten:\n- Datenbank (Chatverlauf, Charaktere, Module, Plugins, Anweisungen, Einstellungen usw.)\n- Charakter-Profilsymbole (Hauptbilder)\n- Benutzersymbole und benutzerdefinierte Hintergründe\n- Persona-Symbole\n- Ordnerbilder\n- Bot-Voreinstellungsbilder\n\n⚠️ NICHT enthalten:\n- Emotionsbilder\n- Zusätzliche Charakter-Assets\n- VITS-Sprachdateien\n- Alle anderen zusätzlichen Mediendateien\n\nDiese Sicherungsdatei enthält NICHT alle Charakter-Assets!\nWenn Sie eine vollständige Sicherung benötigen, verwenden Sie die reguläre lokale Sicherung.\n\nMöchten Sie fortfahren?",
    "partialBackupSecondConfirm": "⚠️⚠️ LETZTE WARNUNG ⚠️⚠️\n\nDiese Sicherung speichert nur Charakter-Profilbilder und speichert die meisten anderen Assets NICHT!\n\nWas gespeichert wird:\n- Datenbank (Chatverlauf, Charaktere, Module, Plugins, Anweisungen, Einstellungen usw.)\n- Charakter-Profilsymbole (Hauptbilder)\n- Benutzersymbole und benutzerdefinierte Hintergründe\n- Persona-Symbole\n- Ordnerbilder\n- Bot-Voreinstellungsbilder\n\nWas NICHT gespeichert wird:\n- Emotionsbilder\n- Zusätzliche Charakter-Assets\n- VITS-Sprachdateien\n- Alle anderen Mediendateien\n\n⚠️ Fahren Sie NUR fort, wenn Sie keine Charakter-Assets in Ihrer Sicherungsdatei benötigen! ⚠️\n\nSind Sie wirklich sicher, dass Sie mit der teilweisen Sicherung fortfahren möchten?",
    "savePartialLocalBackup": "Teilweise Sicherung lokal speichern (ohne Charakter-Assets)",
    "encryptedBackupRefused": "Dieses Backup ist mit RisuAccount verschlüsselt und kann aus technischen Gründen hier nicht gelesen werden. Es wurde nichts importiert.\n\nIm Upstream-RisuAI können Sie diese Daten trotzdem übertragen:\n\n1. Verwenden Sie \"Save Partial Backup Locally (Excluding Character Assets)\". Dieses Backup ist nicht verschlüsselt und behält jeden Chat, einschließlich Kaltlager. Es behält die Profilbilder von Charakteren, Gruppen und Personas, das Benutzersymbol, den Hintergrund sowie Ordner- und Voreinstellungsbilder. Andere Bilder und VITS-Dateien werden weggelassen.\n\n2. Klicken Sie bei der Kontosynchronisierung auf \"Abmelden\" und verwenden Sie dann \"Backup lokal speichern\". Dies behält die .png-Assets, verliert aber die Chats im Kaltlager.\n\nWenn Sie beides tun und beides hier importieren, bleibt alles erhalten. Einzelheiten finden Sie in der Migrationsanleitung.",
    "encryptedBackupImportStopped": "Dieses Backup enthält einen verschlüsselten Teil, und der Import wurde abgebrochen. Einige Bilder oder Kaltlager-Einträge wurden möglicherweise bereits hinzugefügt oder ersetzt. Ihre aktuelle Datenbank wurde nicht verändert.",
    "backupFileUnreadable": "Die Datei konnte nicht gelesen werden. Es wurde nichts importiert.",
    ...
    "bootstrap": {
        ...
        "reportErrorQuestion": "Möchten Sie diesen Fehler an den Entwickler melden?\nDies hilft, das erneute Auftreten desselben Fehlers zu verhindern.\n(Enthält nur Informationen zum beschädigten Modul, keine persönlichen Daten)",
        "diagnosticInformation": (reportData: string) => `# Diagnoseinformationen\n\nBitte kopieren Sie die untenstehenden Informationen und melden Sie sie dem Entwickler über GitHub Issues, Discord oder die Community.\nÜberprüfen Sie die Angaben vorher noch einmal auf sensible Informationen.\n\n\`\`\`json\n${reportData}\n\`\`\``,
        "resetLorebookQuestion": "Möchten Sie NUR die Lore-Buch-Daten in diesem spezifischen beschädigten Modul zurücksetzen?\n(Andere Module und Charakterdaten werden NICHT beeinflusst)\n\nWenn Sie „Nein\" wählen, bleiben die Daten erhalten, aber Fehler können weiterhin auftreten."
    },
    "partialEdit": {
        "editButtonTooltip": "Bearbeiten",
        "deleteButtonTooltip": "Löschen",
        "deleteModalTitle": "Löschbestätigung",
        "deleteConfirmMessage": "Sind Sie sicher, dass Sie diesen Teil löschen möchten?",
        "deleteYes": "Ja",
        "deleteNo": "Nein",
        "editModalTitle": "Teilbearbeitung",
        "matchFound": (method: string) => `Übereinstimmung: ${method}`,
        "matchFailedTitle": "Teilbearbeitung nicht möglich",
        "matchFailedMessage": "Die genaue Position in der ursprünglichen Nachricht konnte nicht gefunden werden. Dies kann bei sehr langen Sätzen oder doppeltem Inhalt vorkommen. Bitte verwenden Sie stattdessen die Funktion zur vollständigen Nachrichtenbearbeitung.",
        "selectMatch": "Wählen Sie aus, welcher Block bearbeitet werden soll",
        "selectDeleteMatch": "Wählen Sie aus, welcher Block gelöscht werden soll",
        "matchesFound": "Übereinstimmungen gefunden",
        "lineNumber": (line: number) => `Zeile ~${line}`,
        "saveShortcut": "Speichern (Strg+Enter)",
    ...
    "confirmInstallPluginViaPlugin": "Sie versuchen, ein Plugin über ein anderes Plugin zu installieren. Dies kann gefährlich sein, wenn das Quell-Plugin nicht vertrauenswürdig ist. Möchten Sie fortfahren?",
    "risuaiProTools": "Risuai Pro-Tools",
    "enableRisuaiProTools": "Risuai Pro-Tools aktivieren",
    "easyPanel": "Easy Panel",
    "mainModel": "Hauptmodell",
    "epEnabled": "Easy Panel-Überschreibung",
    "epRequirementsNotMet": "Easy-Panel-Anforderungen nicht erfüllt. Klicken Sie auf die Schaltfläche unten, um Ihre Einstellungen automatisch für optimale Easy-Panel-Einstellungen anzupassen.",
    "seperateParametersByModel": "Separate Parameter je Modell",
    "disableSeperateParameterChangeOnPresetChange": "Separate Parameteränderung bei Voreinstellungswechsel deaktivieren",
    "keepSessionAlive": "Sitzung aktiv halten",
    "keepSessionAlivePip": "Über PIP",
    "keepSessionAliveSound": "Über Ton",
    ...
    "realmNewListingConfirm": "Dies erstellt einen neuen Realm-Eintrag. Um stattdessen Ihren bestehenden Eintrag zu bearbeiten, melden Sie sich auf der offiziellen RisuRealm-Website an. Fortfahren?",
    "upstreamAgreementPromptBefore": "Diese Funktion nutzt einen Dienst, der vom Upstream-Projekt RisuAI betrieben wird, nicht von dieser App. Um fortzufahren, müssen Sie Folgendes akzeptieren: ",
    "upstreamAgreementTermsOfService": "Nutzungsbedingungen",
    "upstreamAgreementPromptBetween": " und ",
    "upstreamAgreementPrivacyPolicy": "Datenschutzerklärung",
    "upstreamAgreementPromptAfter": ".",
    "upstreamAgreementAccept": "Akzeptieren",
    "upstreamAgreementDecline": "Nicht akzeptieren",
    "upstreamConsentPlaceholder": "Realm-Inhalte stammen von einem Dienst, der vom Upstream-Projekt RisuAI betrieben wird. Stimmen Sie den Bedingungen zu, um sie anzuzeigen.",
    "upstreamConsentShow": "Zustimmen und anzeigen",
