export const languageSpanish = {
    ...
    "errors": {
        ...
        "coldStorageChatLoadFailed": (coldDataKey: string) =>
            `Los datos guardados de este chat no se pudieron cargar (clave: ${coldDataKey}). Puede ser un problema temporal: intenta cambiar de chat o reiniciar. Si el problema persiste, es posible que los datos se hayan perdido y puedas eliminar este chat. Hasta entonces, no ejecutes "limpiar almacenamiento frío".`,
        "coldStorageChatDataMissing": (coldDataKey: string) =>
            `No se pudieron encontrar los datos guardados de este chat (clave: ${coldDataKey}). Si tienes una copia de seguridad de otro dispositivo, restáurala primero. De lo contrario, los datos se han perdido y puedes eliminar este chat.`,
    ...
    "help": {
        ...
        "chatLoadInitialPages": "Cantidad de mensajes recientes que se renderizan al abrir una pantalla de chat. Los valores más altos muestran más historial de inmediato, pero pueden hacer más pesada la apertura de chats largos.",
        "chatLoadAdditionalPages": "Cantidad de mensajes antiguos que se renderizan cada vez que te desplazas hasta arriba. Los valores más altos reducen las cargas repetidas, pero pueden hacer más pesada cada carga.",
        ...
        "v2Calculate": "Evalúa expresiones matemáticas con soporte para aritmética básica (+, -, *, /, %, ^), operadores de comparación (<, >, <=, >=, =, !=), operadores lógicos (&&, ||, !), paréntesis para precedencia y sustitución de variables usando el formato $nombreVariable. Las variables se convierten automáticamente a números (predeterminado a 0 si es inválido).",
        "namespace": "Namespace es un identificador único para el módulo. Se utiliza para prevenir conflictos entre módulos y para la interacción de presets, otros módulos, etc. Si no estás seguro de qué poner, déjalo en blanco.",
        "moduleIntergration": "Puedes habilitar módulos poniendo el namespace del módulo en las secciones de integración de módulos. Si deseas habilitar múltiples módulos, puedes separarlos por comas. Por ejemplo, `modulo1,modulo2,modulo3`. Esto es para usuarios avanzados que desean variar el uso de módulos por presets.",
        ...
        "extractJson": "Si no está en blanco, extraerá datos JSON específicos de la respuesta. Por ejemplo, si deseas extraer `response.text[0]` en la respuesta `{\"response\": {\"text\": [\"hello\"]}}`, puedes poner `response.text.0`.",
        "translatorNote": "Aquí puedes agregar un prompt de traducción único para cada personaje. Esta opción solo se aplica al usar el modelo Ax. para traducción. Para aplicarlo, incluye `{{slot::tnote}}` en la configuración de idioma. No funciona en chats grupales.",
        "groupInnerFormat": "Esto define un formato que se usa en el chat grupal para personajes que no son el hablante. Si no está en blanco, usará este formato en lugar del formato predeterminado. Si `Rol de Otro Bot en Grupo` es `assistant`, también se aplicará al hablante.",
        "groupOtherBotRole": "Esto define un rol que se usa en el chat grupal para personajes que no son el hablante.",
        "chatHTML": "Un HTML que se insertará como cada chat.\n\nPuedes usar CBS y etiquetas especiales.\n- `<risutextbox>`: un cuadro de texto que se usaría para renderizar texto\n- `<risuicon>`: un icono para usuario o asistente\n- `<risubuttons>`: botones de icono para editar chat, traducciones, etc.\n- `<risugeninfo>`: botón de información de generación.",
        ...
        "summarizationPrompt": "El prompt que se utiliza para resumir. Si está en blanco, utilizará el prompt predeterminado. También puedes usar el formato ChatML con {{slot}} para los datos del chat.",
        "translatorPrompt": "El prompt que se utiliza para la traducción. Si está en blanco, utilizará el prompt predeterminado. También puedes usar el formato ChatML con {{slot}} para el idioma de destino, {{slot::content}} para el contenido y {{slot::tnote}} para la nota del traductor.",
        "translateBeforeHTMLFormatting": "Si está habilitado, traducirá el texto antes de los scripts Regex y el formateo HTML. Esto podría reducir los tokens pero podría romper el formateo.",
        "autoTranslateCachedOnly": "Si está habilitado con la opción de Traducción Automática activada, traducirá automáticamente solo los mensajes que el usuario haya traducido previamente.",
        "presetChain": "Si no está en blanco, el preset cambiará y se aplicará aleatoriamente cada vez que el usuario envíe un mensaje en la lista de presets en esta entrada. La lista de presets debe estar separada por comas, por ejemplo, `preset1,preset2`.",
        ...
        "localNetworkModeDesc": "Enruta las URL de modelos privadas/LAN a través de la ruta de ejecución local en lugar del fetch directo del navegador.\n\n**Propósito**\n- Evitar las restricciones de red privada/CORS del navegador para `192.168.x.x`, `10.x.x.x`, `localhost`, `.local` y hosts locales similares.\n- Mitigar el riesgo de tiempo de espera agotado en la inferencia local lenta del primer token en modo de autoalojamiento con Node.\n\n**Cómo funciona**\n- Se aplica solo cuando el Modo de Red Local está activado y la URL de destino se detecta como local/privada.\n- Autoalojamiento con Node: el streaming usa primero el relé experimental Job+WebSocket (con retorno a `/proxy2` si falla); el modo sin streaming usa `/proxy2`.\n- Tauri: usa la ruta nativa/directa.\n- Modo web público: bloqueado para llamadas directas locales/privadas por diseño.\n\n**Restricciones**\n- El alcance se limita a rutas de solicitud compatibles con OpenAI.\n- Esto no evita los límites de origen de Cloudflare entre dos dominios públicos.\n- Usa tu URL de autoalojamiento (donde `globalThis.__NODE__ === true`) para que esta función tenga efecto.",
        "keepSessionAlive": "Mantiene la pestaña activa y evita que la sesión expire por inactividad en los navegadores. Puede requerir actualizar la página para que surta efecto.\n\n- **Por Sonido**: Reproduce un audio silencioso a intervalos regulares para mantener la sesión activa. Este método es conocido por ser el más compatible y efectivo en la mayoría de los navegadores.\n",
        "reSummarizationPrompt": "El prompt utilizado al fusionar varios resúmenes seleccionados en uno mediante edición masiva. Si está en blanco, se utiliza el prompt predeterminado.",
        "hypaV3MemoryTokensRatio": "La fracción del tamaño máximo de contexto asignada al bloque de memoria a largo plazo {{slot}} en el prompt.",
        "hypaV3ExtraSummarizationRatio": "Reduce el umbral en el que se detiene el resumen. En 0, el resumen se detiene tan pronto como los tokens caen por debajo del contexto máximo. Valores más altos provocan más resumen antes de detenerse.",
        "hypaV3MaxChatsPerSummary": "Número máximo de mensajes de chat a incluir al crear un solo resumen.",
        "hypaV3RecentMemoryRatio": "La fracción de tokens de memoria asignada a la memoria reciente. Se completa automáticamente con los resúmenes creados más recientemente hasta llenar los tokens asignados.",
        "hypaV3SimilarMemoryRatio": "La fracción de tokens de memoria asignada a la memoria similar. Se completa automáticamente con los resúmenes que tienen las puntuaciones de similitud más altas con los chats recientes hasta llenar los tokens asignados.",
        "hypaV3RandomMemoryRatio": "Se completa aleatoriamente con resúmenes no seleccionados ya por otras categorías.",
        "hypaV3PreserveOrphanedMemory": "Si está habilitado, se conservarán los resúmenes que hagan referencia a mensajes de chat eliminados. Si está deshabilitado, los resúmenes cuyos mensajes de origen ya no existen se eliminan automáticamente.",
        "hypaV3ProcessRegexScript": "Si está habilitado, los scripts regex se aplicarán a los mensajes de chat de entrada al regenerar resúmenes en el modal de HypaV3.",
        "hypaV3DoNotSummarizeUserMessage": "Si está habilitado, los mensajes del usuario se excluyen del recuento máximo de mensajes por resumen.",
        "hypaV3EnableSimilarityCorrection": "Si está habilitado, se utiliza adicionalmente un resumen de los chats recientes como consulta. No funciona con la HypaMemory V3 experimental.",
        "hypaV3UseExperimentalImpl": "Cambia a la implementación experimental de HypaMemory V3. Habilita la configuración de límite de tasa y cambia el método de consulta.",
        "hypaV3AlwaysToggleOn": "Si está habilitado, el interruptor de HypaMemory se activa automáticamente al seleccionar un personaje.",
        "hypaV3SummarizationRequestsPerMinute": "Número máximo de solicitudes por minuto al modelo de SupaMemory para el resumen. Solo se aplica cuando el modelo de SupaMemory está configurado como Modelo Auxiliar.",
        "hypaV3SummarizationMaxConcurrent": "Número máximo de solicitudes simultáneas al modelo de SupaMemory para el resumen. Solo se aplica cuando el modelo de SupaMemory está configurado como Modelo Auxiliar.",
        "hypaV3EmbeddingRequestsPerMinute": "Número máximo de solicitudes por minuto al modelo de embedding para la búsqueda de similitud.",
    ...
    "chatLoadInitialPages": "Cantidad inicial de carga del chat",
    "chatLoadAdditionalPages": "Cantidad adicional de carga del chat",
    ...
    "streamingDisplayOptimizationMode": "Optimización de visualización en streaming",
    "streamingDisplayOptimizationOff": "Desactivado",
    "streamingDisplayOptimizationBalanced": "Equilibrado",
    "streamingDisplayOptimizationStrong": "Fuerte",
    ...
    "enableBlockPartialEdit": "Activar Edición Parcial por Bloques (Pasa el cursor para editar bloques individuales)",
    "enableDragPartialEdit": "Activar Edición Parcial por Arrastre (Selecciona texto para editar)",
    ...
    "hub": "RisuRealm",
    "hubLoadFailed": "No se pudo cargar RisuRealm. Comprueba tu conexión e inténtalo de nuevo.",
    "hubEmpty": "No se encontraron personajes en RisuRealm en este momento.",
    "hubOffline": "Estás sin conexión. Conéctate a internet para explorar RisuRealm.",
    "hubRetry": "Reintentar",
    "hubBrowseMore": "Explorar RisuRealm",
    "hubDescription": "Explora más personajes con RisuRealm.",
    "hubAnnouncementTitle": "Anuncio de RisuRealm",
    "homeVersion": "Versión",
    "homeLinkDiscordTitle": "Discord",
    "homeLinkDiscordDescription": "Únete a nuestro servidor de Discord para chatear con otros usuarios y con el desarrollador.",
    "homeLinkWebsiteTitle": "Sitio web",
    "homeLinkWebsiteDescription": "Consulta el sitio web oficial del proyecto.",
    "homeLinkGithubTitle": "Código fuente e Incidencias",
    "homeLinkGithubDescription": "Ve el código fuente o reporta una incidencia en GitHub.",
    "homeSourceRepoLabel": "Código fuente",
    "homeSourceIssuesLabel": "Incidencias",
    "homeSourceUpstreamLabel": "upstream",
    "homeLinkEmailTitle": "Correo electrónico",
    "homeLinkEmailDescription": "Ponte en contacto directamente con el desarrollador.",
    "homeSourceEmailForkLabel": "Contactar al mantenedor",
    ...
    "hypaV3Settings": {
        ...
        "queryChatCountLabel": "Cantidad de Chats de Consulta",
        "summaryChunkSeparatorLabel": "Regex Separador de Fragmentos",
    ...
    "hypaV3Modal": {
        "titleLabel": "HypaV3",
        "resetConfirmMessage": "Esta acción no se puede deshacer. ¿Deseas restablecer los datos de HypaV3?",
    ...
    "thinkingType": "Modo de Pensamiento",
    "adaptiveThinkingEffort": "Esfuerzo de Pensamiento Adaptativo",
    ...
    "openAIFlexProcessing": "Procesamiento Flex de OpenAI",
    ...
    "bookmark": "Marcador",
    "duplicatePluginFoundUpdateIt": "Se encontró un plugin duplicado. ¿Deseas actualizar el plugin existente?",
    "branchedText": "Este chat se ha ramificado desde {}.",
    "pluginUpdateFoundInstallIt": "Se ha encontrado una actualización del plugin. ¿Deseas instalar la actualización?",
    "branch": "Rama",
    ...
    "getFullDatabaseConsent": "El plugin {} está solicitando acceso a la base de datos completa, lo que podría exponer información confidencial. ¿Deseas permitir esto?",
    "fetchLogConsent": "El complemento {} está solicitando obtener registros, lo cual podría exponer información confidencial. ¿Deseas permitir esto?",
    ...
    "createFolderOnBranch": "Crear Carpeta al Ramificar",
    ...
    "partialBackupFirstConfirm": "⚠️ ADVERTENCIA: Copia de Seguridad Local Parcial ⚠️\n\nEsta copia de seguridad guarda rápidamente solo la información esencial.\n\nIncluye:\n- Base de datos (historial de chats, personajes, módulos, plugins, prompts, configuración, etc.)\n- Iconos de perfil de personajes (imágenes principales)\n- Iconos de usuario y fondos personalizados\n- Iconos de persona\n- Imágenes de carpetas\n- Imágenes de preset de bot\n\n⚠️ NO Incluye:\n- Imágenes de emociones\n- Activos adicionales de personajes\n- Archivos de voz VITS\n- Todos los demás archivos multimedia adicionales\n\n¡Este archivo de copia de seguridad NO incluye todos los activos de los personajes!\nSi necesitas una copia de seguridad completa, usa la copia de seguridad local normal.\n\n¿Quieres continuar?",
    "partialBackupSecondConfirm": "⚠️⚠️ ADVERTENCIA FINAL ⚠️⚠️\n\n¡Esta copia de seguridad solo guarda las imágenes de perfil de los personajes y NO guarda la mayoría de los demás activos!\n\nQué se guardará:\n- Base de datos (historial de chats, personajes, módulos, plugins, prompts, configuración, etc.)\n- Iconos de perfil de personajes (imágenes principales)\n- Iconos de usuario y fondos personalizados\n- Iconos de persona\n- Imágenes de carpetas\n- Imágenes de preset de bot\n\nQué NO se guardará:\n- Imágenes de emociones\n- Activos adicionales de personajes\n- Archivos de voz VITS\n- Todos los demás archivos multimedia\n\n⚠️ ¡Continúa SOLO si no necesitas los activos de los personajes en tu archivo de copia de seguridad! ⚠️\n\n¿Estás realmente seguro de que quieres continuar con la copia de seguridad parcial?",
    "savePartialLocalBackup": "Guardar Copia de Seguridad Parcial Localmente (Excluyendo Activos de Personajes)",
    "encryptedBackupRefused": "Esta copia de seguridad está cifrada por RisuAccount y no se puede leer aquí por razones técnicas. No se importó nada.\n\nEn la versión upstream de RisuAI, aún puedes trasladar estos datos:\n\n1. Usa \"Save Partial Backup Locally (Excluding Character Assets)\". No está cifrada y conserva todos los chats, incluido el almacenamiento frío. Conserva las imágenes de perfil de personajes, grupos y personas, el icono de usuario, el fondo, y las imágenes de carpetas y de preset. Descarta las demás imágenes y los archivos VITS.\n\n2. Pulsa \"Cerrar Sesión\" en la sincronización de cuenta y luego usa \"Guardar Respaldo Localmente\". Esto conserva los activos .png pero pierde los chats en almacenamiento frío.\n\nHaciendo ambas cosas, e importando ambas aquí, se conserva todo. Consulta la guía de migración para más detalles.",
    "encryptedBackupImportStopped": "Esta copia de seguridad contiene una parte cifrada y la importación se detuvo. Es posible que ya se hayan añadido o reemplazado algunas imágenes o entradas de almacenamiento frío. Tu base de datos actual no se modificó.",
    "backupFileUnreadable": "No se pudo leer el archivo. No se importó nada.",
    "pluginProviderNotFound": "Se detectó un plugin desconocido. Por favor, cambia el modelo o activa el plugin correspondiente.",
    "bootstrap": {
        "dataCorruptionDetected": (name: string, type: string) => `Corrupción de Datos Detectada\n\nEl módulo "${name}" tiene un formato de Libro de Lore dañado.\n\nTipo de dato dañado: ${type}`,
        "reportErrorQuestion": "¿Quieres reportar este error al desarrollador?\nEsto ayuda a evitar que el mismo error vuelva a ocurrir.\n(Solo incluye información del módulo dañado, sin datos personales)",
        "diagnosticInformation": (reportData: string) => `# Información de Diagnóstico\n\nPor favor, copia la información de abajo y repórtala al desarrollador a través de GitHub Issues, Discord o la comunidad.\nVerifica dos veces que no haya información confidencial antes de reportarla.\n\n\`\`\`json\n${reportData}\n\`\`\``,
        "resetLorebookQuestion": "¿Quieres restablecer SOLO los datos del Libro de Lore de este módulo dañado específico?\n(Otros módulos y datos de personajes NO se verán afectados)\n\nElegir \"No\" mantendrá los datos, pero los errores pueden continuar."
    },
    "partialEdit": {
        "editButtonTooltip": "Editar",
        "deleteButtonTooltip": "Eliminar",
        "deleteModalTitle": "Confirmar Eliminación",
        "deleteConfirmMessage": "¿Estás seguro de que quieres eliminar esta parte?",
        "deleteYes": "Sí",
        "deleteNo": "No",
        "editModalTitle": "Edición Parcial",
        "matchFound": (method: string) => `Coincidencia: ${method}`,
        "matchFailedTitle": "No se puede editar parcialmente",
        "matchFailedMessage": "No se pudo localizar con precisión en el mensaje original. Esto puede ocurrir con oraciones muy largas o contenido duplicado. Por favor, usa en su lugar la función de edición completa del mensaje.",
        "selectMatch": "Selecciona qué bloque editar",
        "selectDeleteMatch": "Selecciona qué bloque eliminar",
        "matchesFound": "coincidencias encontradas",
        "lineNumber": (line: number) => `Línea ~${line}`,
        "saveShortcut": "Guardar (Ctrl+Enter)",
    ...
    "confirmInstallPluginViaPlugin": "Estás intentando instalar un plugin a través de otro plugin. Esto puede ser peligroso si el plugin de origen no es confiable. ¿Quieres continuar?",
    "risuaiProTools": "Herramientas Pro de Risuai",
    "enableRisuaiProTools": "Activar Herramientas Pro de Risuai",
    "easyPanel": "Panel Fácil",
    "mainModel": "Modelo Principal",
    "epEnabled": "Anulación del Panel Fácil",
    "epRequirementsNotMet": "No se cumplen los requisitos del Panel Fácil. Haz clic en el botón de abajo para ajustar automáticamente tu configuración a los valores óptimos del Panel Fácil.",
    "seperateParametersByModel": "Parámetros Separados por Modelo",
    "disableSeperateParameterChangeOnPresetChange": "Desactivar Cambio de Parámetros Separados al Cambiar de Preset",
    "keepSessionAlive": "Mantener Sesión Activa",
    "keepSessionAlivePip": "Por PIP",
    "keepSessionAliveSound": "Por Sonido",
    ...
    realmNewListingConfirm: "Esto creará una nueva publicación en Realm. Para editar tu publicación existente, inicia sesión en el sitio propio de RisuRealm. ¿Continuar?",
    upstreamAgreementPromptBefore: "Esta función usa un servicio operado por RisuAI upstream (el proyecto original), no por esta aplicación. Para continuar, acepta sus ",
    upstreamAgreementTermsOfService: "Términos de Servicio",
    upstreamAgreementPromptBetween: " y ",
    upstreamAgreementPrivacyPolicy: "Política de Privacidad",
    upstreamAgreementPromptAfter: ".",
    upstreamAgreementAccept: "Aceptar",
    upstreamAgreementDecline: "No aceptar",
    upstreamConsentPlaceholder: "El contenido de Realm proviene de un servicio operado por RisuAI upstream (el proyecto original). Acepta sus términos para mostrarlo.",
    upstreamConsentShow: "Aceptar y mostrar",
