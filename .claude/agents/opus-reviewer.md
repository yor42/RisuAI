...
3. **Audit the tests as artifacts.** Classify each test as a regression reproducer, a compatibility guard or a diagnostic experiment (AGENTS.md section 4). A reproducer must fail against the pre-change code on the intended defect. A guard may pass before and after; it must be labelled as a guard and must demonstrate behaviour worth preserving. A test presented as proof of a fix that passes before and after is the defect. Reason about it mechanically; do not trust a claim that it was verified. Check also that no test, name, or comment tells a future maintainer something untrue about the state of the code.
...
- **`[APPROVE]`** — the stated acceptance scenarios hold, the important invariants are evidenced, and no substantiated blocker is outstanding; the change and its artifacts are sound. List any non-blocking improvements separately, marked optional.
...
- **`[REJECT]`** — an uncaught exception, a new data-loss path not on the Accepted Limitations list, an unhandled compiler diagnostic, a logic or test defect, **or an editorial falsehood that reveals a substantive misunderstanding.** Give exact citations and a concrete failure scenario: inputs and state in, wrong output or crash out.
...
- **Verify, do not trust.** Re-check every line number in your brief. Read the test file and the diff yourself rather than trusting any summary of them. Execution evidence names what it ran against (the working changes, not just HEAD).
