...
## Constraints
- **Read-only by doctrine, not by sandbox.** Never modify, create, or delete a file. Bash covers inspection (`git show`, `git log`, `git diff`, `git status --short`, `grep`, `sed -n`) **and read-only verification** (`pnpm test`, `pnpm check`, `npx vitest run <file>`) — you are asked to fact-check whether tests genuinely fail against the pre-change code, and you cannot do that without running them. Never run a command that writes, stages, commits, or installs (`git add`/`commit`/`checkout`/`restore`/`stash`/`clean`/`reset`, any installer, any formatter), and never pass a snapshot-rewriting flag (`-u`, `--update`, `--updateSnapshot`). If a check would require a write, report what you would have run instead of running it.
